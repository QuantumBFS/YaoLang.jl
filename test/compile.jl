...
@testset "compile to jl" begin
    ...
    @test compile_to_jl(:r, Control(:k, GateLocation(1, :H))) ==
          :(YaoIR.evaluate!(r, H, $(Position(1)), Locations(k)))
    @test compile_to_jl(:r, Measure(:(1:2))) == :(YaoIR.measure!(r, $(Locations(1:2))))

    @test compile_to_jl(:r, GateLocation(1, :H), :locs) ==
          :(YaoIR.evaluate!(r, H, locs[$(Position(1))]))
    @test compile_to_jl(:r, Control(:k, GateLocation(1, :H)), :locs) ==
          :(YaoIR.evaluate!(r, H, locs[$(Position(1))], locs[Locations(k)]))
