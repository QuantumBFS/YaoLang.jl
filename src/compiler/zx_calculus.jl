...
function IR(circ::ZXDiagram{T,P}) where {T,P}
    ...
    while sum([frontier_v[i] <= length(lo.spider_seq[i]) for i in 1:nqubit]) > 0
        for q in 1:nqubit
            if frontier_v[q] <= length(lo.spider_seq[q])
                ...
                if length(nb) <= 2
                    ...
                    if spider_type(circ, v) == ZXCalculus.SpiderType.Z
                        ...
                        push!(
                            ir,
                            IRTools.Statement(Expr(:quantum, :gate, IRTools.var(length(ir)), q)),
                        )
                ...
                elseif length(nb) == 3
                    ...
                    if sum([
                        findfirst(isequal(u), lo.spider_seq[qubit_loc(lo, u)]) !=
                        frontier_v[qubit_loc(lo, u)] for u in [v, v1]
                    ]) == 0
                        if phase(circ, v) != 0
                            if spider_type(circ, v) == ZXCalculus.SpiderType.Z
                            ...
                            push!(
                                ir,
                                IRTools.Statement(Expr(
                                    :quantum,
                                    :gate,
                                    IRTools.var(length(ir)),
                                    qubit_loc(lo, v),
                                )),
                            )
                        end
