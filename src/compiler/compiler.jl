...
transform(ctx::CompileCtx, ex::Expr) = Expr(ex.head, map(x -> transform(ctx, x), ex.args)...)
...
function codegen_classical(ctx::JuliaASTCodegenCtx, ir::QASTCode)
    ...
    classical_def[:body] =
        :($Circuit{$quoted_name}($(ctx.stub_name), $(Expr(:tuple, ir.free_variables...))))
...
function ctrl_transform(ctx::JuliaASTCodegenCtx, ex::Control)
    ret = transform(ctx, ex.gate)
    push!(
        ret.args, # NOTE: the control location has two part:
        # 1. control locations from the context
        # 2. control locations in the given location space (need flatten it)
        merge_location_ex(ctx.ctrl_locations, flatten_locations(ctx.locations, ex.ctrl_location)),
    )
    return ret
end
